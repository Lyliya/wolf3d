...
** Last update Thu Jan 12 09:21:49 2017 Guillaume
*/
...
void		draw(t_my_framebuffer *f, t_graph *g, t_player_info *player,
		     t_map *m)
{
  t_sprite	weapon;

  weapon.pos_weapon.x = -70;
  weapon.pos_weapon.y = -70;
  weapon.weapon1 = sfTexture_createFromFile("texture/shoot/frame_0.png", NULL);
  weapon.sp1_weapon = sfSprite_create();
  my_draw_sky_ground(f, player->vision);
  my_find_wall(f, *player, m);
  crosshair(f);
  sfSprite_setTexture(g->sprite, g->texture, sfTrue);
  sfSprite_setTexture(weapon.sp1_weapon, weapon.weapon1, sfTrue);
  ...
  sfSprite_setPosition(weapon.sp1_weapon, weapon.pos_weapon);
  sfRenderWindow_drawSprite(g->window, weapon.sp1_weapon, NULL);
